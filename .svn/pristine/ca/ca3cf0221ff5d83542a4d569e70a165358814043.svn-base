...
<el-card shadow="hover">
  <el-container class="main" style="margin-top: 20px">
    <el-table v-loading="loading2"
              ...
              :data="tableData.data"
              style="width: 100%">
      <el-table-column label="还款日" prop="repaymentDate" show-overflow-tooltip></el-table-column>
      <el-table-column label="当前期数" prop="nowStage" show-overflow-tooltip></el-table-column>
      <el-table-column label="当月还款" prop="byStages.monthlySupply" show-overflow-tooltip></el-table-column>
      <el-table-column label="逾期" prop="beOverdue" :formatter="format" show-overflow-tooltip></el-table-column>
      <el-table-column align="right">
        <template slot-scope="scope">
          <el-button size="mini" @click="handleEdit(scope.$index, scope.row)">编辑</el-button>
        </template>
  ...
  new Vue({
    ...
    methods: {
      format: function (row, column) {
        ...
        return row.beOverdue === 1 ? '否' : row.beOverdue === 2 ? '是' : '未知'
      },
      //时间格式化
