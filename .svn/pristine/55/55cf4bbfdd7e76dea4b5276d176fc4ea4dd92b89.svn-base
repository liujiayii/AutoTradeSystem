...
public class ExpenditureServiceImpl implements ExpenditureService {
	...
	@Override
	public String selectExpenditureByDepartmentId(Long id, Integer star, Integer limit) {

		String string;
		try {

			List<Expenditure> result = expenditureDao.selectExpenditureByDepartmentId(id, star, limit);

			if (result.size() >= 1) {
				string = JsonUtil.getResponseJson(1, "查询成功", expenditureDao.selectExpenditureCountByDepartmentId(id),
						result);
			} else {
				string = JsonUtil.getResponseJson(1, "暂无数据", 1, null);
			}
