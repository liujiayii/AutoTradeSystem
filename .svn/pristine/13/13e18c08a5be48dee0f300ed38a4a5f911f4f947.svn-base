...
public class AssessmentServiceImpl implements AssessmentService {
	...
	public String selectAllAssessment(Integer page, Integer limit) {
		...
		try {
			...
			if (AssessmentList.size() <= 0) {
				...
				// 查询到数据返回总数及当前页数据
				string = JsonUtil.getResponseJson(1, "暂无数据", assessmentDao.selectAllCount(), AssessmentList);
			}
	...
	public String selectAssessmentByBrand(String brand) {
		...
		try {
			...
			Assessment selectAssessmentById = assessmentDao.selectAssessmentByBrand(brand);
